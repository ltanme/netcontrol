...
# 检查必需的配置
if [ -z "$ADGUARD_API_URL" ] || [ -z "$ADGUARD_USERNAME" ] || [ -z "$ADGUARD_PASSWORD" ]; then
    echo "错误: AdGuard Home 配置不完整"
    exit 1
fi
...
# 发送 POST 请求的函数
send_post_request() {
    HTTP_CODE=$(curl -o /dev/null -w '%{http_code}' -X POST "$ADGUARD_API_URL" \
    ...
    -H "Cookie: $ADGUARD_SESSION_COOKIE" \
    -H "Host: $ADGUARD_HOST" \
    -H "Origin: http://$ADGUARD_HOST" \
    -H "Referer: http://$ADGUARD_HOST/" \
